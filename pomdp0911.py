...
def exportar_arbol_dot(root, path="arbol_pouct.dot", max_nodes=2000):
    ...
    def _node_label(node):
        visits = _safe_attr(node, ["visits", "N"], 0)
        value = _safe_attr(node, ["value", "V"], None)
        action = _safe_attr(node, ["action", "accion"], None)
        if isinstance(value, (int, float)) and not math.isnan(value):
            label = f"{repr(action) if action else 'Root'}\\nvisits={visits}\\nvalue={value:.3f}"
        else:
            label = f"{repr(action) if action else 'Root'}\\nvisits={visits}"
    ...
    def _walk(node):
        nonlocal counter
        if node is None or counter > max_nodes:
            return None
        nid = f"n{abs(id(node))}"
        if nid in visited:
            return nid
        visited.add(nid)
        counter += 1
        nodes.append((nid, _node_label(node)))
        for c in _safe_attr(node, ["children", "hijos"], []) or []:
            cid = _walk(c)
            if cid:
    ...
    with open(path, "w", encoding="utf-8") as fh:
        fh.write("digraph POUCT {\n  node [shape=box, style=rounded];\n")
        for nid, label in nodes:
            fh.write(f'  {nid} [label="{label}"];\n')
        for a, b in edges:
            fh.write(f"  {a} -> {b};\n")
        fh.write("}\n")
    logging.info(f"[INFO] DOT exportado a {path}")
...
def main():
    ...
    for sim in range(num_simulaciones):
        ...
        try:
            if not use_greedy and hasattr(puct, "root") and puct.root is not None:
                logging.info("[INFO] Imprimiendo árbol POUCT (ASCII)")
                imprimir_arbol_pouct(puct.root, max_depth=3, min_visits=5)

                logging.info("[INFO] Exportando árbol POUCT a arbol_pouct.dot")
                exportar_arbol_dot(puct.root, path=f"arbol_pouct_sim{sim+1}.dot")
 
            else:
                logging.info("[INFO] Árbol POUCT no disponible (use_greedy=True o puct.root es None).")
        except Exception as e:
            logging.exception(f"[ERROR] Al imprimir/exportar árbol POUCT: {e}")
