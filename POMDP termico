...
def crear_belief_desde_sim(sim_state: dict, num_particulas=500, jitter_std=None):
    """
    sim_state ejemplo: {'tdb':24.5,'tr':25.0,'rh':50.0,'Tt':25.0,'vr':0.3,'met':1.0,'clo':0.75}
    jitter_std: dict con desviaciones estándar para las variables; si None se usan valores por defecto.
    """
    ...
    for _ in range(num_particulas):
        ...
        met = sim_state.get('met', 1.0)  # si Simulink entrega met/clo, úsalos directamente
...
class PlanificadorPOUCT_VF(pomdp_py.POUCT):
    ...
    def _plan(self, agent):
        ...
        # Intentar obtener el PMV del estado más probable (MPE)
        try:
...
class ProblemaConfortTermico(pomdp_py.POMDP):
    ...
    def crear(ruido_observacion,
              ...
              num_particulas=1000,
              num_estados=50,
              use_greedy: bool = False):     # ← Nuevo parámetro
        try:
            logging.info("[INFO] Configurando el problema de confort térmico...")

            # 1) Partículas para la creencia inicial
            lista_estados = [
            ...
            # 3) Instanciar ambos planificadores
            ...
            # 4) Elegir según el flag
            planificador_seleccionado = plan_greedy if use_greedy else plan_pouct

            # 5) Crear el agente con el planificador elegido
...
# --- Factory cached para construir el agente usando una creencia dada ---
...
def construir_agente_si_no_existe(belief=None, ruido_observacion=0.5, pmv_deseado=0.0,
    ...
    """
    Construye y devuelve un agente listo para planificar. Si ya existe, devuelve el mismo objeto.
    Si se pasa `belief`, se usa como creencia inicial en lugar de generar partículas aleatorias.
    """
    global _agent_singleton
    if _agent_singleton is not None:
        # Si llamas con una belief distinta y quieres forzar reconstrucción, asigna _agent_singleton = None antes
        return _agent_singleton

    try:
        # 1) Modelos base (usa los constructores que ya tienes en el archivo)
        policy_model      = ModeloDePolitica(pmv_deseado=pmv_deseado)
        transition_model  = ModeloDeTransicion()
        observation_model = ModeloDeObservacion(ruido_observacion=ruido_observacion)
        reward_model      = ModeloDeRecompensa(pmv_deseado=pmv_deseado)
        ...
        # 3) Creencia inicial: usar la que pasan desde Simulink si existe, si no crear por defecto
        if belief is None:
            lista_estados = [
                EstadoConfortTermico.generar_estado_aleatorio()
                for _ in range(num_particulas)
        ...
        # 4) Crear agente personalizado usando la creencia seleccionada
        agente = AgentePersonalizado(
            nombre="AgenteConfortTermico",
            pmv=pmv_deseado,
            belief=creencia_inicial,
        ...
        # Fallback agente simple que mantiene setpoint si falla
...
def compute_action_from_obs(obs):
    # obs: dict con claves tdb,tr,rh,Tt,vr,met,clo (ajusta nombres si usas otros)
    belief = crear_belief_desde_sim(obs, num_particulas=300)
    # construir o recuperar agente (adapta según tu código)
    agent = construir_agente_si_no_existe()  # reemplaza por tu factory
    agent.set_belief(belief)
    accion = agent.plan()  # o el método que devuelva la acción
    # convertir acción a dict simple
    return {'new_Tt': float(getattr(accion,'new_Tt', accion)), 'vr': float(getattr(accion,'vr', 0.0))}

# --- Factory para crear o devolver el agente (singleton) ---
_agent_singleton = None

def construir_agente_si_no_existe(num_particulas=300, planner_params=None):
    """
    Construye y devuelve una instancia del agente POMDP. Si ya fue creada,
    devuelve la misma instancia (cache).
    Se asume que en tu archivo existen definiciones para:
      - TransitionModel (o similar)
      - ObservationModel
      - RewardModel
      - Agent / Planner (pomdp_py.Agent u otra clase)
    Si alguna de esas clases no está definida, la función crea un agente
    fallback que devuelve una acción por defecto.
    """
    global _agent_singleton
    if _agent_singleton is not None:
        return _agent_singleton

    # Intentar construir agente usando tus clases si existen
    try:
        # Ajusta estos nombres si en tu Final2.py usan otros identificadores
        transition_model = ModeloDeTransicion()          # <- define/ajusta según tu código
        observation_model = ModeloDeObservacion()       # <- ya debería existir
        reward_model = ModeloDeRecompensa()                  # <- ya debería existir

        # Inicial belief provisional (vacío); compute_action_from_obs reemplazará la creencia antes de planificar
        init_belief = None

        # Construir planner/agent según tu API (ejemplo genérico con pomdp_py)
        # Si usas otro planner, adapta la línea siguiente
        try:
            ...
            # Si pomdp_py u otra API no encaja exactamente, intenta usar factory propio
            _agent_singleton = AgentePersonalizado(init_belief, transition_model, observation_model, reward_model)  # ajusta
        return _agent_singleton

    except Exception as e:
        # Fallback simple: agente que devuelve acción segura (mantener temperatura)
        class _FallbackAgent:
            def __init__(self, default_new_Tt=24.0, default_vr=0.5):
                self.default_new_Tt = float(default_new_Tt)
                self.default_vr = float(default_vr)
            ...
            def set_belief(self, belief):
                self.belief = belief
            def plan(self):
                # devuelve un objeto simple con atributos new_Tt y vr
                class A:
                    pass
                a = A()
                a.new_Tt = self.default_new_Tt
...
def main():
    ...
    # Flag para activar/desactivar el planificador Greedy
    use_greedy = False

    num_simulaciones = 1
    # Inicializamos resultados para los dos posibles planners;
    # si no usamos Greedy, su lista permanecerá vacía
    ...
    for sim in range(num_simulaciones):
        ...
        # Pasamos use_greedy a la creación del problema
        try:
        ...
        # Definición dinámica de la política de acciones
        ...
        # Solo instanciamos Greedy si use_greedy == True
        planners = {"POUCT": puct}
        if use_greedy:
        ...
        # En caso de creencia vacía, reinyectamos la inicial
        ...
        # Ejecutamos la comparación solo sobre los planners activos
